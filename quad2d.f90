module quad2d_qgaus_mod
  ...
  interface
     function shapefun(x, y, w1, w2, f1, f2)
       use Const, only:SINGLE, DOUBLE
       use Laser
       implicit none
...
contains
  function h(x, w1, w2, f1, f2)
    ...
    do i = 1, size(x)
       xsav = x(i)
       h(i) = qgaus(g, 0d0, xsav, w1, w2, f1, f2)
    end do
  end function h
  function g(y, w1, w2, f1, f2)
    ...
    g = shapefun(xsav, y, w1, w2, f1, f2)
  end function g

  recursive function qgaus(func, a, b, w1, w2, f1, f2)
    real(DOUBLE), intent(in) :: a, b, w1, w2
    type(cos2field), intent(in) :: f1, f2
    complex(DOUBLE) :: qgaus
    interface
       function func(x, w1, w2, f1, f2)
         use Const, only:SINGLE, DOUBLE
         use Laser
         real(DOUBLE), dimension(:), intent(in) :: x
         real(DOUBLE), intent(in) :: w1, w2
    ...
    xr = 0.5d0*(b - a)
    dx(:) = xr*x(:)
    qgaus = xr*sum(w(:)*(func(xm + dx, w1, w2, f1, f2) + func(xm - dx, w1, w2, f1, f2)))
  end function qgaus

  subroutine quad2d_qgaus(x1, x2, ss, w1, w2, f1, f2)
    real(DOUBLE), intent(in) :: x1, x2, w1, w2
    type(cos2field), intent(in) :: f1, f2
    complex(DOUBLE), intent(out) :: ss
    ss = qgaus(h, x1, x2, w1, w2, f1, f2)
...
contains
  ...
  recursive function qromb(func, a, b, w1, w2, f1, f2)
    ...
    integer(SINGLE), parameter :: jmax = 50, jmaxp = jmax + 1, k = 5, km = k - 1
    real(DOUBLE), parameter :: eps = 1d-5
    complex(DOUBLE), dimension(jmaxp) :: h, s
    complex(DOUBLE) :: dqromb
    integer(SINGLE) :: j
    h(1) = 1d0
    s = 0d0
    do j = 1, jmax
       call trapzd(func, a, b, s(j), j, w1, w2, f1, f2)
       if (j >= k) then
          call polint(h(j - km:j), s(j - km:j), DCMPLX(0d0), qromb, dqromb)
  ...
  subroutine quad2d_qromb(x1, x2, ss, w1, w2, f1, f2)
    real(DOUBLE), intent(in) :: x1, x2, w1, w2
    type(cos2field), intent(in) :: f1, f2
    complex(DOUBLE), intent(out) :: ss
    ss = qromb(h, x1, x2, w1, w2, f1, f2)
  end subroutine quad2d_qromb

  recursive subroutine trapzd(func, a, b, s, n, w1, w2, f1, f2)
    ...
    integer(SINGLE) :: it
    if (n == 1) then
       s = 0.5d0*(b - a)*sum(func((/a, b/), w1, w2, f1, f2))
    else
       it = 2**(n - 2)
       del = (b - a)/it
       fsum = sum(func(arth(a + 0.5d0*del, del, it), w1, w2, f1, f2))
       s = 0.5d0*(s + del*fsum)
    end if
  ...
  subroutine polint(xa, ya, x, y, dy)
    ...
    implicit none
    complex(DOUBLE), dimension(:), intent(in) :: xa, ya
    complex(DOUBLE), intent(in) :: x
    complex(DOUBLE), intent(out) :: y, dy
    integer(SINGLE) :: m, n, ns
    complex(DOUBLE), dimension(size(xa)) :: c, d, den, ho
