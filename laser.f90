module Laser
  ...
  implicit none
  private
  public :: gaussfield, trapezoidalfield, cos2field, GetEle, GetEleG, GetEleT, GetEleS, ItoE0
...
contains
  ...
  function GetEleS(pulse, t_in)
    type(cos2field), intent(in) :: pulse
    real(DOUBLE), intent(in) :: t_in
    real(DOUBLE) :: GetEleS
    real(DOUBLE) :: t
    ...
    if (t > tStart .and. t < tEnd) then
       GetEleS = E0*cos((t - tm)*PI/tau)**2*sin(omega*(t - tm) + cep)
    end if
